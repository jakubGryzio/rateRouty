...
class RouteMixinView(generics.RetrieveAPIView):
    ...
    def get(self, request):
        start, end, buffer_value = self.retrieve_request_param(request)
        ...
        winner_idx = self.get_winner_route(buffer_value)  
    ...
    def retrieve_request_param(self, request):
        ...
        return start, end, buffer_value
    
    def get_winner_route(self, buffer_value):
        winner_idx = -1
        winner_rate = -1
        for idx, route in enumerate(self.routes):
            pointsInBuffer = PointsInBuffer(route, buffer_value)
